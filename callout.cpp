...
Callout::Callout(QChart *chart, int nWavfs):
    QGraphicsObject(chart),
    m_chart(chart)
{
    m_selectedStatuColor= QColor(Qt::red);
    m_Wavfs = nWavfs;
}
...
void Callout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    float timeRange = 0.01;
    QLineSeries * tmp = static_cast<QLineSeries *>(m_chart->series()[0]);
    QVector<QPointF> allpoints = tmp->pointsVector();

    float ftimemin = m_anchor.x() - timeRange/2;
    if(ftimemin < 0)
    {
        ftimemin = 0;
    }

    float ftimemax = m_anchor.x() + timeRange/2;
    float ftimeMaxRange = allpoints[allpoints.count() - 1].x();
    if(ftimemax > ftimeMaxRange)
    {
        ftimemax = ftimeMaxRange;
    }

    for(auto & point :allpoints)
    {
        if(m_anchor.y() < point.y() && point.x() >= ftimemin &&  point.x()<= ftimemax)
        {
            m_anchor = point;
        }
    }
    ...
    painter->setPen(m_selectedStatuColor);
    int ir = 10;
    painter->setBrush(Qt::transparent);
    painter->drawEllipse(circlePoint.x()-ir/2,circlePoint.y()-ir/2,ir,ir);
}

void Callout::selectedPoint()
{
    m_selectedStatuColor= QColor(Qt::green);
}
...
void Callout::setTime(const qint64 &time)
{
    m_time = time;
}

void Callout::setAnchor(QPointF point,bool isSelected)
{
    m_anchor = point;
}
